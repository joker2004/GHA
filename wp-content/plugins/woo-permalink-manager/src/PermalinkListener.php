...
class PermalinkListener
{
    ...
    public function replaceTermLink( $link, $term, $taxonomy )
    {
        ...
        return trailingslashit( home_url( $this->buildTermPath( $term, $this->isHierarchical( $this->taxonomyOptions[$taxonomy] ) ) ) );
    }
    ...
    private function getSeoPrimaryTerm( $product )
    {
        ...
        if ( $this->checkSeoPlugin() ) {
            $category = new \WPSEO_Primary_Term( self::WOO_CAT, $product->ID );
    ...
    protected function checkSeoPlugin()
    {
        ...
        $active = function_exists( 'is_plugin_active' ) && defined( 'WPSEO_BASENAME' ) && is_plugin_active( WPSEO_BASENAME ) && class_exists( \WPSEO_Primary_Term::class );
        return $active;
    }
