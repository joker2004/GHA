    ...
    class BeRocket_updater {
        ...
        public static function test_key () {
            if( ! isset($_POST['key']) || ! isset($_POST['id']) ) {
                $data = array('key_exist' => 0, 'status' => 'Failed', 'error' => 'Incorrect query for this function(ID and Key must be sended)');
                ...
                if( $out == FALSE ) {
                    ...
                    if( ! is_wp_error($response) ) {
                        ...
                        $data = array('key_exist' => 0, 'status' => 'Failed', 'error' => $response->get_error_message());
                        $out = json_encode($data);
                    }
                }
            }
